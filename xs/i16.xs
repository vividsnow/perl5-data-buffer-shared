...
ptr_at(SV* self_sv, UV idx)
    CODE:
        EXTRACT_BUF("Data::Buffer::Shared::I16", self_sv);
        void *p = buf_i16_ptr_at(h, (uint64_t)idx);
        if (!p) croak("Data::Buffer::Shared::I16: index out of bounds");
        RETVAL = PTR2UV(p);
    OUTPUT:
        RETVAL
...
new_anon(char* class, UV capacity)
    CODE:
        char errbuf[BUF_ERR_BUFLEN];
        BufHandle* buf = buf_i16_create_anon((uint64_t)capacity, errbuf);
        if (!buf) croak("Data::Buffer::Shared::I16: %s", errbuf[0] ? errbuf : "unknown error");
        RETVAL = sv_setref_pv(newSV(0), class, (void*)buf);
    OUTPUT:
        RETVAL
...
get_raw(SV* self_sv, UV byte_off, UV nbytes)
    CODE:
        ...
        if (!buf_i16_get_raw(h, (uint64_t)byte_off, (uint64_t)nbytes, SvPVX(RETVAL))) {
            SvREFCNT_dec(RETVAL);
            croak("Data::Buffer::Shared::I16: get_raw out of bounds");
        }
    OUTPUT:
        RETVAL
...
set_raw(SV* self_sv, UV byte_off, SV* data_sv)
    CODE:
        ...
        RETVAL = buf_i16_set_raw(h, (uint64_t)byte_off, (uint64_t)dlen, dptr);
    OUTPUT:
        RETVAL
...
cmpxchg(SV* self_sv, UV idx, IV expected, IV desired)
    CODE:
        EXTRACT_BUF("Data::Buffer::Shared::I16", self_sv);
        if (idx >= h->hdr->capacity) croak("Data::Buffer::Shared::I16: index out of bounds");
        RETVAL = newSViv(buf_i16_cmpxchg(h, (uint64_t)idx, (int16_t)expected, (int16_t)desired));
    OUTPUT:
        RETVAL
...
atomic_and(SV* self_sv, UV idx, IV mask)
    CODE:
        EXTRACT_BUF("Data::Buffer::Shared::I16", self_sv);
        if (idx >= h->hdr->capacity) croak("Data::Buffer::Shared::I16: index out of bounds");
        RETVAL = newSViv(buf_i16_atomic_and(h, (uint64_t)idx, (int16_t)mask));
    OUTPUT:
        RETVAL
...
atomic_or(SV* self_sv, UV idx, IV mask)
    CODE:
        EXTRACT_BUF("Data::Buffer::Shared::I16", self_sv);
        if (idx >= h->hdr->capacity) croak("Data::Buffer::Shared::I16: index out of bounds");
        RETVAL = newSViv(buf_i16_atomic_or(h, (uint64_t)idx, (int16_t)mask));
    OUTPUT:
        RETVAL
...
atomic_xor(SV* self_sv, UV idx, IV mask)
    CODE:
        EXTRACT_BUF("Data::Buffer::Shared::I16", self_sv);
        if (idx >= h->hdr->capacity) croak("Data::Buffer::Shared::I16: index out of bounds");
        RETVAL = newSViv(buf_i16_atomic_xor(h, (uint64_t)idx, (int16_t)mask));
    OUTPUT:
        RETVAL
...
new_memfd(char* class, char* name, UV capacity)
    CODE:
        char errbuf[BUF_ERR_BUFLEN];
        BufHandle* buf = buf_i16_create_memfd(name, (uint64_t)capacity, errbuf);
        if (!buf) croak("Data::Buffer::Shared::I16: %s", errbuf[0] ? errbuf : "unknown error");
        RETVAL = sv_setref_pv(newSV(0), class, (void*)buf);
    OUTPUT:
        RETVAL
...
new_from_fd(char* class, int fd)
    CODE:
        char errbuf[BUF_ERR_BUFLEN];
        BufHandle* buf = buf_i16_open_fd(fd, errbuf);
        if (!buf) croak("Data::Buffer::Shared::I16: %s", errbuf[0] ? errbuf : "unknown error");
        RETVAL = sv_setref_pv(newSV(0), class, (void*)buf);
    OUTPUT:
        RETVAL
...
add_slice(SV* self_sv, UV from, ...)
    CODE:
        ...
        for (UV i = 0; i < count; i++)
            tmp[i] = (int16_t)SvIV(ST(i + 2));
        RETVAL = buf_i16_add_slice(h, (uint64_t)from, (uint64_t)count, tmp);
    OUTPUT:
        RETVAL
...
create_eventfd(SV* self_sv)
    CODE:
        ...
        if (RETVAL < 0) croak("Data::Buffer::Shared::I16: eventfd: %s", strerror(errno));
    OUTPUT:
        RETVAL
