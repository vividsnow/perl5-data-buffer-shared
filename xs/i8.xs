...
new(char* class, char* path, UV capacity)
    CODE:
        ...
        BufHandle* buf = buf_i8_create(path, (uint64_t)capacity, errbuf);
...
incr(SV* self_sv, UV idx)
    CODE:
        ...
        RETVAL = newSViv(buf_i8_incr(h, (uint64_t)idx));
    OUTPUT:
        RETVAL
...
decr(SV* self_sv, UV idx)
    CODE:
        EXTRACT_BUF("Data::Buffer::Shared::I8", self_sv);
        if (idx >= h->hdr->capacity) croak("Data::Buffer::Shared::I8: index out of bounds");
        RETVAL = newSViv(buf_i8_decr(h, (uint64_t)idx));
    OUTPUT:
        RETVAL
...
add(SV* self_sv, UV idx, IV delta)
    CODE:
        EXTRACT_BUF("Data::Buffer::Shared::I8", self_sv);
        if (idx >= h->hdr->capacity) croak("Data::Buffer::Shared::I8: index out of bounds");
        RETVAL = newSViv(buf_i8_add(h, (uint64_t)idx, (int8_t)delta));
    OUTPUT:
        RETVAL
...
unlink(SV* self_or_class, ...)
    CODE:
        ...
        if (SvROK(self_or_class)) {
            ...
            if (h) p = h->path;
            else croak("Data::Buffer::Shared::I8: destroyed object");
        } else {
...
ptr_at(SV* self_sv, UV idx)
    CODE:
        EXTRACT_BUF("Data::Buffer::Shared::I8", self_sv);
        void *p = buf_i8_ptr_at(h, (uint64_t)idx);
...
new_anon(char* class, UV capacity)
    CODE:
        char errbuf[BUF_ERR_BUFLEN];
        BufHandle* buf = buf_i8_create_anon((uint64_t)capacity, errbuf);
        if (!buf) croak("Data::Buffer::Shared::I8: %s", errbuf[0] ? errbuf : "unknown error");
        RETVAL = sv_setref_pv(newSV(0), class, (void*)buf);
    OUTPUT:
        RETVAL
...
get_raw(SV* self_sv, UV byte_off, UV nbytes)
    CODE:
        ...
        if (!buf_i8_get_raw(h, (uint64_t)byte_off, (uint64_t)nbytes, SvPVX(RETVAL))) {
            SvREFCNT_dec(RETVAL);
            croak("Data::Buffer::Shared::I8: get_raw out of bounds");
        }
    OUTPUT:
        RETVAL
...
cmpxchg(SV* self_sv, UV idx, IV expected, IV desired)
    CODE:
        EXTRACT_BUF("Data::Buffer::Shared::I8", self_sv);
        if (idx >= h->hdr->capacity) croak("Data::Buffer::Shared::I8: index out of bounds");
        RETVAL = newSViv(buf_i8_cmpxchg(h, (uint64_t)idx, (int8_t)expected, (int8_t)desired));
    OUTPUT:
        RETVAL
...
atomic_and(SV* self_sv, UV idx, IV mask)
    CODE:
        EXTRACT_BUF("Data::Buffer::Shared::I8", self_sv);
        if (idx >= h->hdr->capacity) croak("Data::Buffer::Shared::I8: index out of bounds");
        RETVAL = newSViv(buf_i8_atomic_and(h, (uint64_t)idx, (int8_t)mask));
    OUTPUT:
        RETVAL
...
atomic_or(SV* self_sv, UV idx, IV mask)
    CODE:
        EXTRACT_BUF("Data::Buffer::Shared::I8", self_sv);
        if (idx >= h->hdr->capacity) croak("Data::Buffer::Shared::I8: index out of bounds");
        RETVAL = newSViv(buf_i8_atomic_or(h, (uint64_t)idx, (int8_t)mask));
    OUTPUT:
        RETVAL
...
atomic_xor(SV* self_sv, UV idx, IV mask)
    CODE:
        EXTRACT_BUF("Data::Buffer::Shared::I8", self_sv);
        if (idx >= h->hdr->capacity) croak("Data::Buffer::Shared::I8: index out of bounds");
        RETVAL = newSViv(buf_i8_atomic_xor(h, (uint64_t)idx, (int8_t)mask));
    OUTPUT:
        RETVAL
...
new_memfd(char* class, char* name, UV capacity)
    CODE:
        char errbuf[BUF_ERR_BUFLEN];
        BufHandle* buf = buf_i8_create_memfd(name, (uint64_t)capacity, errbuf);
        if (!buf) croak("Data::Buffer::Shared::I8: %s", errbuf[0] ? errbuf : "unknown error");
        RETVAL = sv_setref_pv(newSV(0), class, (void*)buf);
    OUTPUT:
        RETVAL
...
new_from_fd(char* class, int fd)
    CODE:
        char errbuf[BUF_ERR_BUFLEN];
        BufHandle* buf = buf_i8_open_fd(fd, errbuf);
        if (!buf) croak("Data::Buffer::Shared::I8: %s", errbuf[0] ? errbuf : "unknown error");
        RETVAL = sv_setref_pv(newSV(0), class, (void*)buf);
    OUTPUT:
        RETVAL
